#!/usr/bin/env python3
"""
Script mejorado para procesar archivos HTML antiguos de Les Luthiers
y generar páginas modernas con estructura JSON - Versión 2
"""
...
class ContentExtractor(HTMLParser):
    """Extractor personalizado de contenido HTML mejorado"""
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'h1':
            ...
            self.in_p = True
            self.content_parts.append('\n<p>')
        elif tag == 'br' and self.in_content and (self.in_p or self.in_h5 or self.in_h6):
            ...
            # Filtrar enlaces de navegación no deseados
            if href and not href.startswith('javascript:') and not href.startswith('#'):
                # Convertir enlaces internos
                if href.endswith('.htm'):
                    href = href.replace('.htm', '.html')

                # Filtrar enlaces a navegación antigua
                if any(x in href for x in ['../index.htm', '../nav/', '../contacto/', '../galerias/',
                                           '../discos/', '../anecdotario/', '../partituras/',
                                           '../lbe/', '../hca/', '../ve/', '../ocho/', '../hg/',
                                           '../ladm/', '../dtodo/', '../rr/', '../proyectos/', '../menu/']):
                    # Ignorar estos enlaces de navegación antigua
            ...
            # Filtrar imágenes de navegación/decorativas
            skip_images = ['spacer.gif', 'd2_r', 'i_t16_', 'i_t17_', 'i_t18_', 'i_t19_',
                          'i_t21_', 'i_t22_', 'i_t23_', 'd2_', 'i_r2_c12']

            if src and not any(skip in src for skip in skip_images):
                # Actualizar ruta de imagen solo para imágenes de contenido real
                orig_src = src
                ...
                self.images.append(orig_src.split('gfx/')[-1] if 'gfx/' in orig_src else orig_src)

                # Construir tag de imagen con atributos
                img_attrs = [f'src="{src}"']
                if alt:
                    img_attrs.append(f'alt="{html.escape(alt)}"')
                if width and int(width) > 50:  # Solo preservar dimensiones de imágenes grandes
    ...
    def get_content(self):
        ...
        # Limpiar espacios múltiples dentro de líneas
        ...
        # Limpiar saltos de línea antes de tags de cierre
        content = re.sub(r'\s+</p>', '</p>', content)
        content = re.sub(r'\s+</h[1-6]>', lambda m: m.group(0).strip(), content)
...
def generate_html_page(title, content, output_file):
    """Genera una página HTML moderna"""
    # Determinar si es la página principal
    is_index = output_file == "index.html"

    breadcrumb = '' if is_index else '<div class="section-header">\n                    <p><a href="index.html">← Les Luthiers</a></p>\n                </div>\n                '
    ...
    <div class="site-container">
        ...
        <div class="content-wrapper">
            ...
            <main class="main-content">
                {breadcrumb}<div class="section-content">
{content}
                </div>
            </main>
        </div>
...
def process_files():
    ...
    print("Procesando archivos de Les Luthiers - Versión 2...\n")
    ...
    for source_file, title, output_file in FILES_TO_PROCESS:
        ...
        html_output = generate_html_page(title, content, output_file)
    ...
    print(f"Imágenes de contenido referenciadas: {len(total_images)}")
    print(f"\nArchivos generados:")
    ...
    for page in all_pages:
        print(f"  - {page['file']} - {page['title']} ({page['images_count']} imágenes)")
